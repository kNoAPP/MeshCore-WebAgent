...
/**
 * The Neighbors tab: the repeater's up-to-8 most recently heard nodes, read
 * via the `neighbors` CLI command and parsed by {@link parseNeighborsReply}.
 * Neighbors that resolve to a located contact/advert are plotted on
 * {@link NeighborsMap} with an SNR-labeled link from the repeater; the rest
 * stay in the list below so none silently disappears (issue #178). Each row
 * shows the node (a contact's name when known), its age, and SNR. Admins get a
 * per-row Remove (inline confirm) that sends `neighbor.remove <prefix>` for
 * that exact prefix — never a blank/space prefix, which the firmware treats as
 * "remove all". Refresh lives on the map when shown, else in the list. Fetches
 * once on entry and again on demand.
 */
function NeighborsTab({
  ...
  // otherwise there's nothing to anchor or draw, so the list stands alone.
