 ...
 * per session once useMeshCore has the session's key (`deriveChannelKey`),
 * and again when the session re-keys after a channel change.
 */
 ...
 * session key (`deriveChannelKey`) rather than inventing a second one.
 *
 * @remarks
